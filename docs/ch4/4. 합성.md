...
이러한 합성에 대한 예제를 우리는 1장에서 잠시 살펴봤다.
...
선택 조합기는 함수 2개를 인수로 받아 값이 있으면 첫 번째 함수의 결과를 없으면 두 번째 함수의 결과를 반환한다.

``` javascript
// javascript
const R = require('ramda');

const add = R.curry((a, b) => console.log(a + b));
const alt = R.curry((func1, func2, value) => value ? func1(value) : func2(value));
...
add = pydash.curry(lambda a, b: print(a + b))
alt = pydash.curry(lambda func1, func2, value: func1(value) if value else func2(value))
