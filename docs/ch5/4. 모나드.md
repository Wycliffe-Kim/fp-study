...
앞에서 말했던 바처럼 함수 체인과 합성은 함수형 프로그래밍에서 아주 중요한 개념이라고 했다. 또한 우리가 이 장에서 보았던 것처럼 에러 처리를 위해서는 모나드가 필수적이다. 꼭 에러 처리를 위한 것이 아니더라도, 값에 안전하게 접근하여 처리하기 위해서는 모나드는 필수적이다. 이 두 가지를 합치면 어마어마한 시너지 효과를 낼 수 있다.
...
showStudent('444-44-4444').orElse(console.log); // ID가 444444444인 객체를 찾을 수 없습니다.
```
...
show_student('444-44-4444').or_else(print) # ID가 444444444인 객체를 찾을 수 없습니다.
```

다음은 합성과 모나드를 합친 코드이다.
